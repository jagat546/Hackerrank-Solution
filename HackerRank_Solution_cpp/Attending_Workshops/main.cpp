...
struct Available_Workshops {
    int iWorkShopCnt;       // Number of workshops
    vector<WorkShop> vWorkShops;     // workshop
};
...
int CalculateMaxWorkshops(Available_Workshops* p_pAvWS) {
    // Sort by the time each workshop ends
    std::sort(p_pAvWS->vWorkShops.begin(), p_pAvWS->vWorkShops.end(), [](WorkShop A, WorkShop B) {
        return A.iEndTime < B.iEndTime;
    });
    
    // In alignment
    // If the start time of the current workshop is greater than the previous end time OK, EndTime renewal
